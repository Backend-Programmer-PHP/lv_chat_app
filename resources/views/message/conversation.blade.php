                        ...
                        <ul class="list-unstyled chat-list mt-2 mb-0">
                            @if ($users->count())
                                @foreach ($users as $user)
                                    <li class="clearfix @if($user->id == $friendInfo->id) active @endif" onclick="location.href='{{ route('message.conversation', $user->id) }}';">
                                        @php
                                            $img = App\Helpers\Helper::makeImageFromName($user->name);
                                        @endphp
